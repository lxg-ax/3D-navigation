...
* 描述符（20×60 极坐标俯视图）在建图阶段每个关键帧算一遍，body 帧描述符落盘到 `sc_db.bin`
...
* 这套门控同时解决两类问题：长走廊 / 对称楼道 SC 误匹配（spatial gate 杀掉），被搬运 / 漂错楼层 MCL 卡死（持续不一致触发重启）
...
* 定位链：`dddnav_preflight` 做启动期 QoS / TF / 参数自检 → `sc_global_init` 给初始位姿 + 在线 watchdog → MCL 收敛 → `slam_health_monitor` + `nav_perf_monitor` 在线监控
