...
// ROS-free fork of hku-mars/STD STDesc.h.
// Two changes from upstream:
//   1. Drop ros::NodeHandle / ros::Publisher / visualization_msgs from the
//      core API. Callers (LIO-SAM mapOptimization, sc_global_init) build
//      ROS 2 markers / load params from rclcpp themselves.
//   2. Use Ceres LocalParameterization (≤ 2.0) instead of Manifold (≥ 2.1),
//      so we can compile against the libceres-dev shipped in Ubuntu 22.04.
// Algorithmic code is untouched. Original copyright in src/STDesc.cpp.
...
// NOTE: read_parameters(ros::NodeHandle&, ConfigSetting&) and
// publish_std_pairs(..., ros::Publisher&) from the upstream API have been
// stripped — see header comment. Callers populate ConfigSetting fields
// directly and translate STD pairs to whichever marker library they prefer.
