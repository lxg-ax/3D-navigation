...
void STDescManager::SearchLoop(
  ...
  if (stds_vec.size() == 0) {
    std::cerr << "[STDescManager] No STDescs!" << std::endl;
...
void STDescManager::PlaneGeomrtricIcp(
  ...
  kd_tree->setInputCloud(input_cloud);
  // Ceres 2.0 (Ubuntu 22.04 default) does not have ceres::Manifold yet —
  // it ships LocalParameterization. 2.1+ has both, but Manifold is the
  // forward path. Pick the right one at compile time so this builds on
  // either version.
  ...
  problem.SetParameterization(para_q, quaternion_manifold);
#endif
