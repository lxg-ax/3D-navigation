...
namespace
{
std::string strip_yaml_string_quotes(std::string s)
{
...
std::string resolve_perception_share_path(const std::string & p)
{
  if (p.empty() || p.front() == '/') {
    return p;
  }
  const std::string share = ament_index_cpp::get_package_share_directory("perception_3d");
...
{ 
  ...
  */
  ...
  no_entry_zone_pcd_file_dir_ = resolve_perception_share_path(no_entry_zone_pcd_file_dir_);
...
void NoEntryLayer::parseYAML(std::string path){
  ...
  for(auto i=zones.begin();i!=zones.end();i++){
    ...
    if(strstr((*i).get_name().c_str(), std::string(".pcd").c_str())){
      std::string unit_zone_string = resolve_perception_share_path(strip_yaml_string_quotes((*i).value_to_string()));
      zone_read = true;
      //split by /
