...
if [ "$d_bag" != "${d_bag#[Yy]}" ] ;then 
  echo "Download bag"
  cd ~/dddnav_bags && curl -L -c cookies.txt 'https://drive.usercontent.google.com/uc?export=download&id='1bKBosqJjREsddwJh0jMrOVxGpI9pLXwb \
...
if [ "$m_bag" != "${m_bag#[Yy]}" ] ;then 
  echo "Download bag"
  cd ~/dddnav_bags && curl -L -c cookies.txt 'https://drive.usercontent.google.com/uc?export=download&id='1MnvlZkYrqKZwCJ74LMuCD1zh4owUq6I- \
      | sed -rn 's/.*confirm=([0-9A-Za-z_]+).*/\1/p' > confirm.txt
...
if [ "$u_bag" != "${u_bag#[Yy]}" ] ;then 
  echo "Download bag"
  cd ~/dddnav_bags && curl -L -c cookies.txt 'https://drive.usercontent.google.com/uc?export=download&id='1QVMqS-rCi5Q96LgwVDxc-jD5VhVjwohn \
      | sed -rn 's/.*confirm=([0-9A-Za-z_]+).*/\1/p' > confirm.txt
