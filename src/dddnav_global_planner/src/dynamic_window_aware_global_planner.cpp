...
void DWA_GlobalPlanner::handle_accepted(const std::shared_ptr<rclcpp_action::ServerGoalHandle<dddnav_sys_core::action::GetPlan>> goal_handle)
{
  rclcpp::Rate r(20);
...
void DWA_GlobalPlanner::initial(const std::shared_ptr<perception_3d::Perception3D_ROS>& perception_3d, 
  ...
  this->action_server_global_planner_ = rclcpp_action::create_server<dddnav_sys_core::action::GetPlan>(
    this,
    "/get_dwa_plan",
...
void DWA_GlobalPlanner::makePlan(const std::shared_ptr<rclcpp_action::ServerGoalHandle<dddnav_sys_core::action::GetPlan>> goal_handle){
  ...
  if(!perception_3d_ros_->getSharedDataPtr()->is_static_layer_ready_){
    ...
    threading_timer_->cancel();
    auto result = std::make_shared<dddnav_sys_core::action::GetPlan::Result>();
