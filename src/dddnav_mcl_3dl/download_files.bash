...
if [ "$d_bag" != "${d_bag#[Yy]}" ] ;then 
  echo "Download bag"
  cd ~/dddnav_bags && curl -L -c cookies.txt 'https://drive.usercontent.google.com/uc?export=download&id='1NV3Jy6e3fRioA6hmc5snBQJiidK6K694 \
  ...
  cd ~/dddnav_bags && curl -L -c cookies.txt 'https://drive.usercontent.google.com/uc?export=download&id='1NX9gEqrZVGHI1vMm2SBlqBZ3MF-rLEyJ \
      | sed -rn 's/.*confirm=([0-9A-Za-z_]+).*/\1/p' > confirm.txt
