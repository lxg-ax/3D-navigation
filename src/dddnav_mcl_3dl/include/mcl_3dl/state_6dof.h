...
public:
  ...
  static State6DOF generateNoise(RANDOM_ENGINE& engine, const NOISE_GEN& gen)
  {
    ...
    {
      printf("----------Dimension of noise must be 6. Passed: %lu ----------\n", gen.getDimension());
    }
    State6DOF noise;
...
inline void NoiseGeneratorBase<float>::setMean(const State6DOF& mean)
{
  ...
  if (mean.isDiff())
  {
    printf("---------- Failed to generate noise. mean must be mcl_3dl::Quat. ----------\n");
  }
  for (size_t i = 0; i < 3; i++)
  {
    mean_[i] = mean[i];
...
inline void DiagonalNoiseGenerator<float>::setSigma(const State6DOF& sigma)
{
  ...
  if (!sigma.isDiff())
  {
    printf("---------- Failed to generate noise. sigma must be rpy vec. ----------\n");
  }
  for (size_t i = 0; i < 3; i++)
  {
    sigma_[i] = sigma[i];
