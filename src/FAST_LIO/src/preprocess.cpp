...
void Preprocess::mid360_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  // Use real timestamps from Livox driver (nanoseconds stored as double → milliseconds offset)
  double t0 = pl_orig.points[0].timestamp;  // first point timestamp (ns as double)
  given_offset_time = true;
  ...
  for (uint i = 0; i < plsize; ++i)
  {
    ...
    added_pt.z = pl_orig.points[i].z;
    added_pt.intensity = pl_orig.points[i].intensity;
    // curvature stores time offset in ms (used by sync_packages and IMU undistortion)
    added_pt.curvature = (pl_orig.points[i].timestamp - t0) * 1e-6;  // ns → ms
