...
void ImageProjection::zPitchRollFeatureRemoval() {
  ...
  {
    // 手动变换避免 pcl::transformPointCloud 对含 NaN 点云触发 SIGFPE
    Eigen::Matrix4f tf_mat = trans_lidar2horizontal_af3.matrix().cast<float>();
    _full_cloud_no_pitch.resize(_full_cloud->size());
    for (size_t k = 0; k < _full_cloud->size(); ++k) {
      const auto& src = _full_cloud->points[k];
      auto& dst = _full_cloud_no_pitch.points[k];
      if (!std::isfinite(src.x)) {
        dst = src;  // 保留 NaN 点不变
      } else {
        Eigen::Vector4f pt(src.x, src.y, src.z, 1.0f);
        Eigen::Vector4f res = tf_mat * pt;
        dst.x = res(0); dst.y = res(1); dst.z = res(2);
