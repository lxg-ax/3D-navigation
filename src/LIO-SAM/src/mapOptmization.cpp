...
public:
    ...
    rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr subFastLioOdom;
    rclcpp::Subscription<sensor_msgs::msg::PointCloud2>::SharedPtr subFastLioCloud;

    std::deque<nav_msgs::msg::Odometry> gpsQueue;
    nav_msgs::msg::Odometry latestFastLioOdom;
    bool fastLioOdomReceived = false;
    bool useFastLioFrontend = true; // FAST-LIO2 as frontend, bypass LIO-SAM scan matching
    
    // FAST-LIO2 odometry queue for keyframe selection
    std::deque<nav_msgs::msg::Odometry> fastLioOdomQueue;
    std::mutex mtxFastLioOdom;
    
    // FAST-LIO2 registered cloud for keyframe storage
    pcl::PointCloud<PointType>::Ptr latestFastLioCloud;
    std::mutex mtxFastLioCloud;
    bool fastLioCloudReceived = false;

    // FAST-LIO2 covariance from EKF P matrix (forwarded via odometry message)
    Eigen::Matrix<double, 6, 6> fastLioCov;
    bool fastLioCovValid = false;
    
    // Degeneracy detection
    bool frontendDegenerate = false;
    double degeneracyEigenThreshold = 50.0;  // eigenvalue threshold for degeneracy
    ...
    mapOptimization(const rclcpp::NodeOptions & options) : ParamServer("lio_sam_mapOptimization", options)
    {
        ...
        subFastLioOdom = create_subscription<nav_msgs::msg::Odometry>(
            "/Odometry", qos,
            std::bind(&mapOptimization::fastLioOdomHandler, this, std::placeholders::_1));
        
        // Subscribe to FAST-LIO2 registered cloud (body frame)
        subFastLioCloud = create_subscription<sensor_msgs::msg::PointCloud2>(
            "/cloud_registered_body", qos,
            std::bind(&mapOptimization::fastLioCloudHandler, this, std::placeholders::_1));
    ...
    void allocateMemory()
    {
        ...
        latestFastLioCloud.reset(new pcl::PointCloud<PointType>());
        ...
        matP.setZero();
        fastLioCov.setIdentity();
        fastLioCov *= 1e-4;  // small initial covariance
    }
    ...
    void laserCloudInfoHandler(const lio_sam::msg::CloudInfo::SharedPtr msgIn)
    {
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval)
        {
            ...
            if (useFastLioFrontend)
            {
                // FAST-LIO2 path: use its odometry directly, skip scan matching
                if (!updatePoseFromFastLio())
                {
                    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000, 
                        "FAST-LIO2 odometry not available yet, waiting...");
                    return;
                }

                // Store the FAST-LIO2 registered cloud as keyframe cloud
                // We split it into corner/surf using curvature for compatibility with loop closure
                {
                    std::lock_guard<std::mutex> cloudLock(mtxFastLioCloud);
                    if (fastLioCloudReceived && latestFastLioCloud->size() > 0)
                    {
                        splitFastLioCloudToFeatures(latestFastLioCloud, laserCloudCornerLast, laserCloudSurfLast);
                        fastLioCloudReceived = false;
                    }
                }
    ...
    // Downsample FAST-LIO2 cloud for keyframe storage. All points go to surf
    // since GICP doesn't need corner/surf separation.
    void splitFastLioCloudToFeatures(
        pcl::PointCloud<PointType>::Ptr cloudIn,
        pcl::PointCloud<PointType>::Ptr cornerOut,
        pcl::PointCloud<PointType>::Ptr surfOut)
    {
        cornerOut->clear();
        surfOut->clear();

        if ((int)cloudIn->size() < 20)
            return;

        pcl::PointCloud<PointType>::Ptr cloudDS(new pcl::PointCloud<PointType>());
        pcl::VoxelGrid<PointType> voxelFilter;
        voxelFilter.setInputCloud(cloudIn);
        voxelFilter.setLeafSize(0.2f, 0.2f, 0.2f);
        voxelFilter.filter(*cloudDS);

        if ((int)cloudDS->size() < 20)
        {
            *surfOut = *cloudIn;
            return;
        }
    ...
    // Set transformTobeMapped from FAST-LIO2 odometry queue
    // Uses linear interpolation for better time alignment
    bool updatePoseFromFastLio()
    {
        std::lock_guard<std::mutex> lock(mtxFastLioOdom);
        
        if (fastLioOdomQueue.empty())
            return false;

        // Find the two closest FAST-LIO2 odom messages bracketing current laser time
        nav_msgs::msg::Odometry odomBefore, odomAfter;
        double timeBefore = -1e9, timeAfter = 1e9;
        bool foundBefore = false, foundAfter = false;

        for (const auto& odom : fastLioOdomQueue)
        {
            double odomTime = stamp2Sec(odom.header.stamp);
            if (odomTime <= timeLaserInfoCur && odomTime > timeBefore)
            {
                timeBefore = odomTime;
                odomBefore = odom;
                foundBefore = true;
            }
            if (odomTime > timeLaserInfoCur && odomTime < timeAfter)
            {
                timeAfter = odomTime;
                odomAfter = odom;
                foundAfter = true;
        ...
        // Purge old messages
        while (!fastLioOdomQueue.empty() && 
               stamp2Sec(fastLioOdomQueue.front().header.stamp) < timeLaserInfoCur - 1.0)
        {
            fastLioOdomQueue.pop_front();
        }

        // Fallback: use nearest if interpolation not possible
        nav_msgs::msg::Odometry selectedOdom;
        if (foundBefore && foundAfter && (timeAfter - timeBefore) < 0.2)
        {
            // Linear interpolation
            double ratio = (timeLaserInfoCur - timeBefore) / (timeAfter - timeBefore);
            
            tf2::Quaternion qBefore, qAfter;
            tf2::fromMsg(odomBefore.pose.pose.orientation, qBefore);
            tf2::fromMsg(odomAfter.pose.pose.orientation, qAfter);
            tf2::Quaternion qInterp = qBefore.slerp(qAfter, ratio);

            selectedOdom.pose.pose.position.x = odomBefore.pose.pose.position.x + ratio * (odomAfter.pose.pose.position.x - odomBefore.pose.pose.position.x);
            selectedOdom.pose.pose.position.y = odomBefore.pose.pose.position.y + ratio * (odomAfter.pose.pose.position.y - odomBefore.pose.pose.position.y);
            selectedOdom.pose.pose.position.z = odomBefore.pose.pose.position.z + ratio * (odomAfter.pose.pose.position.z - odomBefore.pose.pose.position.z);
            selectedOdom.pose.pose.orientation = tf2::toMsg(qInterp);
        }
        else if (foundBefore && (timeLaserInfoCur - timeBefore) < 0.5)
        {
            selectedOdom = odomBefore;
        }
        else if (foundAfter && (timeAfter - timeLaserInfoCur) < 0.5)
        {
            selectedOdom = odomAfter;
        }
        else if (foundBefore || foundAfter)
        {
            // Last resort: use whichever is closest
            double distBefore = foundBefore ? (timeLaserInfoCur - timeBefore) : 1e9;
            double distAfter = foundAfter ? (timeAfter - timeLaserInfoCur) : 1e9;
            if (distBefore < distAfter && distBefore < 1.0)
                selectedOdom = odomBefore;
            else if (distAfter < 1.0)
                selectedOdom = odomAfter;
            else
                return false;
        }
        ...
        // Extract pose
        tf2::Quaternion q;
        tf2::fromMsg(selectedOdom.pose.pose.orientation, q);
        ...
        transformTobeMapped[3] = selectedOdom.pose.pose.position.x;
        transformTobeMapped[4] = selectedOdom.pose.pose.position.y;
        transformTobeMapped[5] = selectedOdom.pose.pose.position.z;

        // Update incremental odometry tracking
        incrementalOdometryAffineFront = trans2Affine3f(transformTobeMapped);
        incrementalOdometryAffineBack = trans2Affine3f(transformTobeMapped);

        // Extract FAST-LIO EKF covariance and detect degeneracy
        {
            const auto& srcOdom = (foundBefore && foundAfter && (timeAfter - timeBefore) < 0.2) ? odomBefore : selectedOdom;
            const auto& cov = srcOdom.pose.covariance;
            // Reconstruct 6x6 covariance: FAST-LIO maps row i → P[i<3 ? i+3 : i-3]
            for (int i = 0; i < 6; ++i)
            {
                for (int j = 0; j < 6; ++j)
                {
                    int pi = i < 3 ? i + 3 : i - 3;
                    int pj = j < 3 ? j + 3 : j - 3;
                    fastLioCov(pi, pj) = cov[i * 6 + j];
                }
            }
            fastLioCovValid = true;

            // Degeneracy detection via position covariance eigenvalues
            Eigen::Matrix3d posCov = fastLioCov.block<3, 3>(3, 3);
            Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> solver(posCov);
            double maxEigen = solver.eigenvalues().maxCoeff();
            double minEigen = solver.eigenvalues().minCoeff();
            
            frontendDegenerate = false;
            if (minEigen > 1e-10)
            {
                double conditionNumber = maxEigen / minEigen;
                if (conditionNumber > degeneracyEigenThreshold || maxEigen > 0.1)
                {
                    frontendDegenerate = true;
                    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 5000,
                        "Degeneracy detected! Condition: %f, MaxEigen: %f", conditionNumber, maxEigen);
                }
            }
        }
    ...
    void fastLioOdomHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        latestFastLioOdom = *odomMsg;
        fastLioOdomReceived = true;
        
        // Queue for keyframe pose lookup
        std::lock_guard<std::mutex> lock(mtxFastLioOdom);
        fastLioOdomQueue.push_back(*odomMsg);
        // Keep queue bounded
        while (fastLioOdomQueue.size() > 500)
            fastLioOdomQueue.pop_front();
    }

    void fastLioCloudHandler(const sensor_msgs::msg::PointCloud2::SharedPtr cloudMsg)
    {
        std::lock_guard<std::mutex> lock(mtxFastLioCloud);
        latestFastLioCloud->clear();
        pcl::fromROSMsg(*cloudMsg, *latestFastLioCloud);
        fastLioCloudReceived = true;
    }
    ...
    void performLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        // Priority: 1) External, 2) Scan Context, 3) Distance-based
        if (detectLoopClosureExternal(&loopKeyCur, &loopKeyPre) == false)
            if (detectLoopClosureScanContext(&loopKeyCur, &loopKeyPre) == false)
                if (detectLoopClosureDistance(&loopKeyCur, &loopKeyPre) == false)
                    return;

        // extract cloud - use full merged clouds for GICP
        pcl::PointCloud<PointType>::Ptr cureKeyframeCloud(new pcl::PointCloud<PointType>());
        pcl::PointCloud<PointType>::Ptr prevKeyframeCloud(new pcl::PointCloud<PointType>());
        {
            // Current keyframe: single frame
            loopFindNearKeyframes(cureKeyframeCloud, loopKeyCur, 0);
            // Previous keyframe: local submap for robust matching
        ...
        // GICP loop closure verification (coarse-to-fine)
        static pcl::GeneralizedIterativeClosestPoint<PointType, PointType> gicp;
        gicp.setMaxCorrespondenceDistance(4.0);
        ...
        {
            // Fine alignment failed, try with relaxed distance first
            gicp.setMaxCorrespondenceDistance(8.0);
            gicp.align(*unused_result);
            if (gicp.hasConverged() == false || gicp.getFitnessScore() > historyKeyframeFitnessScore)
                return;
            // Refine with tight distance
            gicp.setMaxCorrespondenceDistance(2.0);
            gicp.setMaximumIterations(32);
            gicp.align(*unused_result, gicp.getFinalTransformation());
            if (gicp.hasConverged() == false || gicp.getFitnessScore() > historyKeyframeFitnessScore)
                return;
        }
        ...
        {
            gtsam::Pose3 poseFromOdom = pclPointTogtsamPose3(copy_cloudKeyPoses6D->points[loopKeyCur]);
            gtsam::Pose3 relativeOdom = poseFromOdom.between(poseTo);
            gtsam::Pose3 relativeLoop = poseFrom.between(poseTo);
            gtsam::Pose3 discrepancy = relativeOdom.between(relativeLoop);
            
            double transDiff = discrepancy.translation().norm();
            Eigen::Matrix3d rotMat = discrepancy.rotation().matrix();
            double trace = rotMat.trace();
            double rotDiff = std::acos(std::min(1.0, std::max(-1.0, (trace - 1.0) / 2.0)));
            
            double maxTransCorrection = historyKeyframeSearchRadius * 0.5;
            double maxRotCorrection = M_PI / 6.0;  // 30 degrees
            if (transDiff > maxTransCorrection || std::abs(rotDiff) > maxRotCorrection)
            {
                RCLCPP_WARN(get_logger(), 
                    "Loop rejected by consistency check: trans=%.2f (max=%.2f), rot=%.2f deg (max=30)",
                    transDiff, maxTransCorrection, rotDiff * 180.0 / M_PI);
                return;
            }
        }
    ...
    bool saveFrame()
    {
        ...
        {
            // Time-based keyframe for Livox, but still require minimum motion
            // to avoid redundant keyframes when stationary
            if (timeLaserInfoCur - cloudKeyPoses6D->back().time > 1.0)
            {
                ...
                pcl::getTranslationAndEulerAngles(transBetween, xt, yt, zt, rollt, pitcht, yawt);
                // Require at least some minimal motion (10% of normal threshold)
                if (sqrt(xt*xt + yt*yt + zt*zt) > surroundingkeyframeAddingDistThreshold * 0.1 ||
                    abs(rollt) > surroundingkeyframeAddingAngleThreshold * 0.1 ||
                    abs(pitcht) > surroundingkeyframeAddingAngleThreshold * 0.1 ||
                    abs(yawt) > surroundingkeyframeAddingAngleThreshold * 0.1)
                    return true;
            }
        }
    ...
    void addOdomFactor()
    {
        if (cloudKeyPoses3D->points.empty())
        {
            ...
            if (fastLioCovValid)
            {
                // Adaptive noise from FAST-LIO EKF covariance
                // Extract diagonal variances, apply minimum floor and scale factor
                gtsam::Vector Vector6(6);
                // GTSAM convention: [rot_x, rot_y, rot_z, trans_x, trans_y, trans_z]
                double scaleFactor = frontendDegenerate ? 100.0 : 10.0;  // inflate more during degeneracy
                double minRotVar = frontendDegenerate ? 1e-2 : 1e-4;
                double minTransVar = frontendDegenerate ? 0.1 : 1e-2;
                
                for (int i = 0; i < 3; ++i)
                {
                    Vector6(i) = std::max(fastLioCov(i, i) * scaleFactor, minRotVar);     // rotation
                    Vector6(i + 3) = std::max(fastLioCov(i + 3, i + 3) * scaleFactor, minTransVar); // translation
                }
                odometryNoise = noiseModel::Diagonal::Variances(Vector6);
            }
            else
            {
                // Fallback: fixed noise
    ...
    void publishOdometry()
    {
        ...
        // Publish TF: map → odom
        // map→odom = T_map_lidar * T_odom_lidar^-1
        if (fastLioOdomReceived)
        {
            tf2::Quaternion q_map_lidar;
            q_map_lidar.setRPY(transformTobeMapped[0], transformTobeMapped[1], transformTobeMapped[2]);
            tf2::Transform T_map_lidar(q_map_lidar,
                tf2::Vector3(transformTobeMapped[3], transformTobeMapped[4], transformTobeMapped[5]));

            tf2::Transform T_odom_lidar;
            tf2::fromMsg(latestFastLioOdom.pose.pose, T_odom_lidar);

            tf2::Transform T_map_odom = T_map_lidar * T_odom_lidar.inverse();

            geometry_msgs::msg::TransformStamped map2odom;
            map2odom.header.stamp = timeLaserInfoStamp;
            map2odom.header.frame_id = mapFrame;
            map2odom.child_frame_id = odometryFrame;
            map2odom.transform = tf2::toMsg(T_map_odom);
