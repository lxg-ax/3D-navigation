...
public:
    ...
    bool publishTF_ = true;  // 是否广播 odom→base_link TF
    ...
    TransformFusion(const rclcpp::NodeOptions & options) : ParamServer("lio_sam_transformFusion", options)
    {
        ...
        // 是否广播 odom→base_link TF（与 FastLIO 联用时设为 false，由 FastLIO 负责）
        declare_parameter("publishTF", true);
        get_parameter("publishTF", publishTF_);
    }
...
public:
            ...
            ParamServer("lio_sam_imu_preintegration", options)
    {
        ...
        priorBiasNoise  = gtsam::noiseModel::Isotropic::Sigma(6, 1.0); // very relaxed for MID360 IMU
    ...
    void odometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        ...
        if (systemInitialized == false)
        {
            ...
            try {
                ...
                RCLCPP_WARN(get_logger(), "GTSAM init failed: %s, retrying...", e.what());
                resetOptimization();
                resetParams();
                return;
            }
        ...
        if (key == 100)
        {
            ...
            try {
                ...
                RCLCPP_WARN(get_logger(), "GTSAM reset optimization failed, resetting params...");
                resetOptimization();
                resetParams();
                return;
            }
        ...
        try {
            ...
            RCLCPP_WARN(get_logger(), "GTSAM optimization failed: %s, resetting...", e.what());
            resetOptimization();
            resetParams();
            return;
        }
        ...
        try {
            ...
            RCLCPP_WARN(get_logger(), "GTSAM calculateEstimate failed, resetting...");
            resetOptimization();
            resetParams();
            return;
        }
